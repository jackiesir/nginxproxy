...
# 因为nginx每天23:59分会做一次备份,旧的access.log会被临时删掉，一旦删掉该监控程序将会失效(问题暂时还不能通过python自身解决),只能通过把如下语句加入Nginx备份脚本配合,定时重启python程序,但能解决问题就OK了。

# sudo ps aux | grep tailtest.py | grep -v "grep" | awk -F ' ' '{print $2}' | xargs kill -9
#
